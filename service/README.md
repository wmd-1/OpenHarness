...
> **双跑范围说明**：Celery 后端跑完整的「崩溃 + reclaim + fencing」；Temporal 后端当前只验证
> happy-path fencing 接线。其「worker 崩溃 + Activity 重试」复用 `running` 任务的 reclaim 尚未
> 实现——`claim()` 仅认 `QUEUED`/`RETRYING`（见 §3.2 与 `app/workers/tasks.py`），而 Temporal 无
> beat 把 `running` 任务翻回 `RETRYING`，故重试无法重占。该缺口待补：Temporal 重试前需先把任务翻
> 回 `RETRYING`（或由 Temporal 侧负责 reclaim），届时即可在 Temporal 下也跑「崩溃 + fencing」。
