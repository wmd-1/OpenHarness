...
## 6. Phase 3 端到端 e2e（Docker，不依赖 oh-e2e:latest）

Phase 3 的集成 e2e **完全独立于** scale-multi-instance 的 `oh-e2e:latest` 中间镜像：被测镜像
由 `Dockerfile.e2e.phase3` **直接基于真实 OpenHarness 基础镜像** 构建，仅覆盖 Phase 3 的
`service/` + 一个离线 `oh` 渲染桩（`e2e/oh_stub.sh`）+ 租户 seed 脚本 + `OH_ROLE` 入口。
