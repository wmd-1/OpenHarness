...
`reclaim`（旧 owner 被判死、重派）。Temporal Activity 重试（同一 workflow 实例、跨 worker
崩溃恢复）**视为一次 reclaim**：`_reset_for_reclaim` 先把任务翻回 `RETRYING` 并
`lease_token+1`，随后 `claim` 再 `+1`，因此重试后的写持有严格更高的 token，旧 owner 的写被
fence。同一 worker 进程内的纯本地重试（不跨 ownership 转移）沿用同一 token。Celery 与 Temporal
两条路径遵守同一条 bump 规则（都由 beat / 重试触发真正的 ownership 转移）。
...
- Temporal 路径的 reclaim 现由 Activity 重试驱动：`_reset_for_reclaim` 在 attempt>1 时复位任务，
  survivor worker 重新 `claim` 并完成，fence 保证与 Celery 一致（见 §6 双跑 e2e）。
...
## 6. Phase 3 端到端 e2e（Docker）

Phase 3 的集成 e2e 被测镜像由 `Dockerfile.e2e.phase3` **基于 scale-multi-instance 的
`oh-e2e:latest` 镜像** 构建（该镜像已含 `oh` CLI、headless chrome 及 Phase 3 从 R7–R13 继承的
全部 scale 依赖：boto3 / prometheus / structlog / opentelemetry / psutil / asyncpg 等）。本层
仅补装 Phase 3 独有的 `temporalio` + `limits`，再叠加**当前** Phase 3 `service/`、离线 `oh`
渲染桩（`e2e/oh_stub.sh`）与扩展了 `seed` / `temporal-worker` 角色的 `OH_ROLE` 入口。
`BASE_IMAGE` 可被 build-arg 覆盖（如指向裸基础镜像），默认 `oh-e2e:latest` 以复用其依赖层。

```bash
# 在具备 Docker daemon 的环境（本地 / CI）运行：
bash e2e/run_e2e_phase3.sh
```

脚本会：
1. 构建 `oh-e2e-phase3:latest`；
2. 起多副本栈（api×2 / worker×2 / beat + postgres / redis / minio(S3)）；
3. 跑 **跨租户隔离** 断言（`X-API-Key` 鉴权、跨租户读 404、缺 key 401、`/healthz` 免鉴权）；
4. 跑 **lease fencing** 断言（真实杀掉占用 worker → beat reclaim → 另一 worker 重占并完成，
   `video_lease_fence` 恰好 1 行、accepted_token == 终态 lease_token、storage_key == output_path）；
5. 用 `--profile temporal` 重新起栈（temporal-server + temporal-worker），对**同一套断言做
   双跑**（隔离 + happy-path fencing），证 Celery / Temporal 两套后端行为等价。

> **双跑范围说明**：Celery 与 Temporal 后端**均**跑完整的「崩溃 + reclaim + fencing」。Temporal 下
> 由 Activity 重试驱动 reclaim——Activity 带 `heartbeat_timeout`（默认 30s），worker 崩溃后 Temporal
> 在超时内检测到并依 `RetryPolicy`（`maximum_attempts=3`）在存活的 `temporal-worker` 上重跑；重试
> （attempt>1）先经 `_reset_for_reclaim` 把 `running` 任务翻回 `RETRYING` 并提升 fence token，再
> `claim` 重占，终态与产物均被 fence。双副本 `temporal-worker` + `temporal-worker2` 保证有 survivor
> 接单（见 `docker-compose.e2e.phase3.yml`）。
