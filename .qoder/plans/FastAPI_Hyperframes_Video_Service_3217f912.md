...
可选增强：解析 `Range` 请求头返回 206，便于浏览器拖拽进度。

## 9. SSE 进度推送 (`/events`)

worker 每读到一行 stdout，就 `LPUSH oh:logs:<task_id> <line>` 并 `PUBLISH`。FastAPI 端 `EventSourceResponse` 订阅 channel + 起始重放 list 历史，最后 `event: done` 结束。
