...
- **Worker 异步路径的 tenant 上下文（关键，避免 RLS 在 worker 侧失效）**：HTTP 请求经中间件 `SET LOCAL app.current_tenant` 即可；但 **worker 没有 request context**。因此渲染 worker 在从 DB `claim` / 读回 task 后，必须在开启 DB 事务/连接时显式 `SET LOCAL app.current_tenant = :task.tenant_id`（使用 task 自身携带的 `tenant_id`，而非全局变量），之后该 task 的所有 DB 访问（写终态、写 `audit_log`、读产物元数据）才受 RLS 约束。若采用「集中查询层」退化方案，则 worker 同样在 Repository 层绑定 `tenant_id`，语义相同。无论 RLS 还是集中层，**tenant 上下文来源都必须是被 claim 的 task 行本身**，杜绝 worker 借用其它租户上下文。
...
> **Lease 语义定义（实现一致性前提）**：`lease_token` 表示 **task execution ownership（执行所有权）**，而非 workflow / 本地 retry。
> - token **仅在 ownership 转移时变化**：claim（首次派发）、reclaim（owner 被判定死亡后重派）、或显式移交。
> - token **不因同一 owner 的本地 retry / Temporal Activity retry 而变化**——同一 owner 的反复执行持有同一个 token，因此 fence 比较不会产生「自己拒绝自己」。
> - 此定义同时约束 Celery 与 Temporal 两条路径：无论渲染由谁执行，lease_token 的 bump 时机只有上述三种，避免两条路径语义漂移。

### 8.1 机理（升级 Phase 2 §11）

- `video_tasks` 加 `lease_token bigint`；**migration 初始值 `BIGINT NOT NULL DEFAULT 0`**（历史任务/未 claim 任务 `lease_token=0` 视为「无 fencing」，首次 claim 从 `0 + 1 = 1` 起，杜绝 `NULL + 1` 语义歧义）。claim/reclaim 时**原子自增**（`UPDATE ... SET lease_token = lease_token + 1 ... RETURNING lease_token`）；`claim()` 改为返回 `(claimed: bool, token: int)`，worker 进程内存持有当前 `token`（见 D3）。
- **token 生命周期 / bump 时机（Celery 与 Temporal 须一致）**：token **仅在 ownership 转移时变化**——首次 claim（新 owner）、reclaim（旧 owner 死亡后重派给新 owner）。**同一 owner 的本地 retry、Temporal Activity 自动重试（同一 workflow 实例内）均不 bump**（持有同一 token，见 §8 语义定义）；Temporal 路径下只有 workflow 被取消并经 beat/独立 reclaim 重派新 owner 才 bump。两条路径遵守同一规则，避免 fence 语义漂移。
- **heartbeat / 续约也携带 token（防旧 owner 假活阻塞 reclaim）**：worker 存活刷新 `UPDATE ... SET heartbeat_at=now() WHERE worker_id=:wid AND lease_token=:token` 必须带 token；旧 owner 复苏若以 stale token 心跳，该 `UPDATE` 命中 0 行，beat 不会误判为 alive，从而正常触发 reclaim（否则旧 owner 持续心跳会使 reclaim 永久不触发、fence 失效）。续约失败 N 次 → 主动放弃并触发 reclaim。
- 每次写携带 token：
...
- **跨后端一致性（重要，对应 F5，已拍板）**：本 WS 的 lease fence 依赖 reclaim 来 bump `lease_token`；而 reclaim 与心跳存活注册当前**耦合在 Celery**（`beat.py` 的 `worker_process_init` 信号启动 liveness 线程、Celery beat 周期任务 `recover_lost_tasks`）。若 `OH_SCHEDULER_BACKEND=temporal`（WS-B）下不运行 Celery 渲染 worker，则无人 bump token，本 WS 的「严格 lease」保证在 Temporal 路径下不成立。**已决策（采用方向 2，非「仅限 Celery」固化方案）**：将 `recover_lost_tasks` / watch-dog 抽象为**与调度后端无关的独立 reclaim 组件**——先定接口与职责，初版仍由 Celery 调用（确保 Celery 路径行为与 Phase 2 完全一致），随后让 Temporal Activity 复用同一套 reclaim/lease 逻辑，**避免维护两套实现**。这样既不阻塞当前 WS-B/WS-C 开发，也不把「Strict Lease 仅支持 Celery」固化为长期架构；R20 的严格保证据此对两种后端都成立。
...
- `test_ws_c_fencing.py`：
  ...
  - **heartbeat 携带 token（防假活）**：旧 owner 以 stale token 调 heartbeat → 命中 0 行；beat 在 `heartbeat_at` 超期后仍正常 reclaim（不被旧 owner 心跳误导为 alive）。
